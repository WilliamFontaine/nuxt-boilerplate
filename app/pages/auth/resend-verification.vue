      ...
      <UPageCard class="shadow-xl border-0 bg-white dark:bg-neutral-900">
        ...
        >
          <template #description>
            {{ t('auth.resendVerification.links.alreadyVerified') }}
            <ULink :to="localePath('/auth/login')" class="text-primary font-medium">
              {{ t('auth.resendVerification.links.backToLogin') }} </ULink
            >.
          </template>
...
const handleSubmit = async (event: FormSubmitEvent<ForgotPasswordData>) => {
  try {
    ...
    if (response.statusCode === 200) {
      ...
      success({
        title: t('auth.resendVerification.messages.success.title'),
        message: t('auth.resendVerification.messages.success.message')
      })
    ...
    switch (errorCode) {
      ...
      case ERROR_CODES.RATE_LIMIT.TOO_MANY_ATTEMPTS:
        error({
          title: t('auth.resendVerification.messages.error.title'),
          message: t('auth.resendVerification.messages.error.tooManyRequests', {
      ...
      case ERROR_CODES.USER.NOT_FOUND:
        error({
          title: t('auth.resendVerification.messages.error.title'),
          message: t('auth.resendVerification.messages.error.userNotFound')
        })
        break

      case ERROR_CODES.USER.INVALID_DATA:
        error({
          title: t('auth.resendVerification.messages.error.title'),
          message: t('auth.resendVerification.messages.error.alreadyVerified')
        })
        break
      ...
      default:
        error({
          title: t('auth.resendVerification.messages.error.title'),
          message: t('auth.resendVerification.messages.error.generic')
        })
    }
  } finally {
